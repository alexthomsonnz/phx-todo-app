defmodule PhxTodoWeb.ItemControllerTest do
  use PhxTodoWeb.ConnCase
  alias PhxTodo.Todo

  import PhxTodo.TodoFixtures
  ...
  describe "edit item" do
    ...
    test "renders form for editing chosen item", %{conn: conn, item: item} do
      ...
      assert html_response(conn, 200) =~ "placeholder=\"Edit item\""
    end
  end
  ...
  describe "toggle updates the status of an item 0 > 1 | 1 > 0" do
    ...
    test "toggle_status/1 item.status 1 > 0", %{item: item} do
      ...
      # first toggle
      toggled_item = %{item | status: PhxTodoWeb.ItemController.toggle_status(item)}
      assert toggled_item.status == 1
      # second toggle sets status back to 0
      assert PhxTodoWeb.ItemController.toggle_status(toggled_item) == 0
    end
